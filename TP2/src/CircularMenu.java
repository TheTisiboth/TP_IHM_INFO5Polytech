			...
			if (list.size() <= 8) {
				...
				for (int i = 8; i < list.size(); i++) {
					...
					list.get(i).setBounds(
							(int) (xCentre - taille / 2),
							(int) (yCentre + radius_border + 5 + 45*(i - 8)),
							taille, 40);
					add(list.get(i));
				}
			}
	...
	public void paintComponent(Graphics g) {
		...
		if (list.size() <= 8) {
			...
			for (int i = 8; i < list.size(); i++) {
				...
				g2d.drawRect((int) (center.getX() - taille / 2), (int) (center.getY() + radius_border + 5 + 45*(i - 8)), taille, 40);
			}
		}
	...
	public MenuItem identifySection(Point clickPoint) {
		...
		if (list.size() <= 8) {
			...
			if(dist > radius_border) {
				...
				for (int i = 8; i < list.size(); i++) {
					taille = list.get(i).getLength() + 10;
					if((cx-taille/2)<=px && px<=(cx+taille/2) && (cy+radius_border+5+45*(i-8))<=py && py<=(cy+radius_border+5+45*(i-7))) {
						index = i;
					}
				}
				if(index == -1){
