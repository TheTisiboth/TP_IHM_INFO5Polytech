...
public class MarkingMenu extends JPanel {
	...
	public MarkingMenu(ArrayList<MenuItem> items) {
		init(items, 100);
		itemsPlacement();
		addListener();
	}

	public MarkingMenu(ArrayList<MenuItem> l, int radiusT) {
		init(l, radiusT);
		itemsPlacement();
		addListener();
	}
	...
	public void draged(MouseEvent e) {
		MenuItem section = identifySection(new Point(e.getX(), e.getY() - 75));
		if (section != null) {
			section.doClick(e.getX(), e.getY());
		}
	}
